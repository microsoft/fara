...
def _get_base_eval_arg_parser():
    ...
    parser.add_argument('--max_rounds', type=int, default=75, help='Maximum trajectory length')
    parser.add_argument('--web_surfer_model_type', type=str, choices=["fara"], default='fara', help='WebSurfer model type (default: fara)')
    parser.add_argument('--model_port', type=int, default=5000, help='Model port (default: 5000)')
    parser.add_argument('--device_id', type=str, default="0", help='Device ID (default: 0)')
    parser.add_argument('--redo_eval', action='store_true', help='Redo evaluation even if already exists')
    ...
    parser.add_argument('--dtype', type=str, choices=['auto', 'half', 'float16', 'bfloat16', 'float', 'float32'], default='auto', help='Data type for VLLM model (default: auto). Anything besides float32 may cause instability in model outputs -- leading to json parsing errors of tool calls')
    ...
    parser.add_argument('--eval_only', action='store_true', help='Only evaluate existing results without running new examples')
